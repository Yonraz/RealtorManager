...
export const generateToken = (userId: ObjectId) => {
  ...
  try {
    if (!JWT_SECRET || !JWT_REFRESH_EXPIRATION) {
      throw new HttpError(
        "JWT secret key or refresh expiration not found",
        500
      );
    }
...
export const verifyToken = (token: string) => {
  let payload: any;
  try {
    if (JWT_REFRESH_SECRET) payload = jwt.verify(token, JWT_REFRESH_SECRET);
    else throw new HttpError("JWT secret key not found", 500);
  } catch (err) {
    return false;
  }
  return payload;
